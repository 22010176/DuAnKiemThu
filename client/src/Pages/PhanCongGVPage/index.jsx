...
const PhanCongGVPage = () => {
  ...
  const columns = [
    ...
    { title: 'Kỳ/Năm', key: 'kyNam', width: 120, render: (record) => `${record.ky} - ${record.namHoc}` },
    { title: 'SV', dataIndex: 'soSinhVien', key: 'soSinhVien', width: 60, align: 'center' },
    { title: 'Tín chỉ', dataIndex: 'soTinChi', key: 'soTinChi', width: 70, align: 'center' },
    {
    ...
    {
      title: 'Giảng viên', key: 'giangVien', width: 200,
      render: (record) => (
        record.giangVien ? (
          <Space>
    ...
    <div style={{ padding: '24px', backgroundColor: '#f5f5f5' }}>
          ...
          <Row justify="space-between" align="middle">
            <Col>
              <Space>
                ...
                <Button type="primary" icon={<UserAddOutlined />} disabled={selectedRowKeys.length === 0} onClick={handleAssignTeacher}>
      ...
      {/* <Card size="small" style={{ marginBottom: '16px', backgroundColor: '#fafafa' }}>
        <Row gutter={16} align="middle">
          <Col span={6}>
            <Select placeholder="Chọn khoa" allowClear style={{ width: '100%' }} value={filters.khoa} onChange={(value) => handleFilterChange('khoa', value)}>
              {khoaOptions.map(khoa => <Option key={khoa} value={khoa}>{khoa}</Option>)}
            </Select>
          </Col>
          <Col span={6}>
            <Select placeholder="Chọn năm học" style={{ width: '100%' }} value={filters.namHoc} onChange={(value) => handleFilterChange('namHoc', value)}>
              {namHocOptions.map(nam => <Option key={nam} value={nam}>{nam}</Option>)}
            </Select>
          </Col>
          <Col span={6}>
            <Select placeholder="Chọn kỳ" style={{ width: '100%' }} value={filters.ky} onChange={(value) => handleFilterChange('ky', value)}>
              {kyOptions.map(ky => <Option key={ky} value={ky}>{ky}</Option>)}
            </Select>
          </Col>
          <Col span={6}>
            <Alert message={`Hiển thị ${filteredData.length} lớp đang mở`} type="info" showIcon style={{ textAlign: 'center' }} />
          </Col>
        </Row>
      </Card> */}
